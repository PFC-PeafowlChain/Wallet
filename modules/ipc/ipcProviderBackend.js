/**
The IPC provider backend filter and tunnel all incoming request to the PFC node.
...
class IpcProviderBackend {
                ...
                return new Q((resolve, reject) => {
                  const onStateChange = newState => {
                    if (ethereumNode.STATES.CONNECTED === newState) {
                      ...
                      log.debug(
                        `PFC node connected, resume connecting socket ${ownerId}`
                      );
  ...
  /**
   * Handler for when PFC node state changes.
   *
   * Auto-reconnect sockets when PFC node state changes
   *
  ...
  _onNodeStateChanged(state) {
    switch (state) { // eslint-disable-line default-case
      ...
      case ethereumNode.STATES.STOPPING:
        log.info('PFC node stopping, disconnecting sockets');
